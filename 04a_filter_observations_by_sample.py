...
def filter_observations_by_sample(input_path, output_path, threshold, as_fraction):
    table = load_table(input_path)
    
    def filter_function(values, id_, metadata):
        return np.sum(values) >= threshold

    filtered_table = table.filter(filter_function, inplace=False)
    filtered_table.to_hdf5(output_path, "Filtered Table")
...
def main():
    ...
    for abundances in input_table.iter_data():
        if as_fraction:
            abundance_fractions = abundances.astype(float) / np.sum(abundances)
            indices = [i for (i, j) in enumerate(abundance_fractions > threshold) if not j]
        else:
            indices = [i for (i, j) in enumerate(abundances > threshold) if not j]
